...
def audit_repository(root: Path) -> dict[str, Any]:
    ...
    checks = [
        ("RW001", "README", _exists(root, "README*"), 12, "목적·설치·사용·지원 방법이 포함된 간결한 README를 추가하세요."),
        ("RW002", "오픈소스 라이선스", _exists(root, "LICENSE*", "COPYING*"), 12, "OSI 승인 라이선스 파일을 추가하고 README에도 표시하세요."),
        ("RW003", "기여 안내", _exists(root, "CONTRIBUTING*", ".github/CONTRIBUTING*"), 10, "개발 환경, 테스트, 리뷰 기준과 예제 데이터 정책을 문서화하세요."),
        ("RW004", "행동 강령", _exists(root, "CODE_OF_CONDUCT*", ".github/CODE_OF_CONDUCT*"), 6, "커뮤니티 행동 강령과 집행 절차를 추가하세요."),
        ("RW005", "보안 정책", _exists(root, "SECURITY*", ".github/SECURITY*"), 8, "취약점을 비공개로 신고하는 방법을 설명하세요."),
        ("RW006", "이슈 템플릿", _exists(root, ".github/ISSUE_TEMPLATE/*"), 7, "버그와 기능 제안을 위한 이슈 양식을 추가하세요."),
        ("RW007", "Pull Request 템플릿", _exists(root, ".github/PULL_REQUEST_TEMPLATE*", ".github/pull_request_template*"), 6, "테스트·범위·보안 영향을 확인하는 PR 체크리스트를 추가하세요."),
        ("RW008", "지속적 통합", _exists(root, ".github/workflows/*.yml", ".github/workflows/*.yaml"), 12, "최소 권한으로 push와 PR에서 테스트를 실행하세요."),
        ("RW009", "자동 테스트", _exists(root, "tests/*", "test/*", "__tests__/*"), 10, "합성 예제를 사용하는 빠른 테스트를 추가하세요."),
        ("RW010", "의존성 잠금 파일", _exists(root, "package-lock.json", "pnpm-lock.yaml", "yarn.lock", "poetry.lock", "uv.lock", "Pipfile.lock"), 5, "생태계에 적합한 의존성 잠금 파일을 커밋하세요."),
        ("RW011", "설치·사용 문서", (any("install" in section or "getting started" in section or "설치" in section for section in sections) and any("usage" in section or "run" in section or "사용" in section for section in sections)), 7, "README에 분명한 설치 및 사용 제목을 추가하세요."),
        ("RW012", "실행 가능한 테스트 명령", commands["test_command"], 5, "전체 테스트를 실행하는 하나의 명령을 문서화하세요."),
    ]
    results = [
