...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit open-source contributor readiness.")
    ...
    if args.format == "json":
        ...
        lines.extend(f"[{'PASS' if check['passed'] else 'MISS'}] {check['id']} {check['name']}" for check in report["checks"])
        rendered = "\n".join(lines)
    if args.output:
